...
// use id from $clean_response['categoryid'] if set, otherwise use default category
if (isset($clean_response['categoryid']) && $clean_response['categoryid'] > 0) {
    $category_id = $clean_response['categoryid'];
} else {
    // Get default question category for the course    
    $helper = new helper();
    $category_id = $helper->get_default_question_category($clean_response['courseid']);
}
